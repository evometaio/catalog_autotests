...
class QubeLocators(BaseProjectLocators):
    ...
    class Arisha:
        ...
        # Локаторы для информации об апартаменте (не модалка, а статичная информация)
        class ApartmentInfo:
            ...
            # Заголовок с номером апартамента (внутри контейнера)
            APARTMENT_NUMBER = (
                '.ant-card:has-text("APT. 104"), ._info_1k7zz_45:has-text("APT. 104")'
            )

            # Информация о типе (поддерживаем оба варианта)
            ...
            # Информация о этаже (может отсутствовать на продакшене)
            FLOOR_INFO = (
                '.ant-card:has-text("Floor:"), ._info_1k7zz_45:has-text("Floor:")'
            )
            FLOOR_VALUE = (
            ...
            # Информация о площади (поддерживаем оба формата)
