...
@pytest.mark.ui
@pytest.mark.parametrize("panorama_type", ["rotation", "yard", "lobby-k1", "lobby-k2", "lobby-k3"])
def test_mark_360_area_tour(mark_page, panorama_type):
    ...
    if panorama_type == "rotation":
        ...
        with allure.step("Проверяем, что URL изменился (добавился hash с типом панорамы)"):
            current_url = mark_page.get_current_url()
            assert f"#tour3d={panorama_type}" in current_url or f"tour3d={panorama_type}" in current_url, (
