...
def test_cubix_download_pdf_on_agent_page(cubix_agent_page):
    ...
    try:
        with allure.step("Открываем агентскую страницу проекта Cubix"):
            cubix_agent_page.open_agent_page()

            current_url = cubix_agent_page.get_current_url()
        ...
        with allure.step("Ищем и кликаем на первый доступный апартамент"):
            selected_apartment = cubix_agent_page.find_and_click_available_apartment()
        ...
        with allure.step("Кликаем на кнопку Sales Offer и скачиваем PDF"):
            ...
            success, file_path = cubix_agent_page.download_pdf_and_verify()
        ...
        if downloaded_file_path:
            with allure.step("Удаляем скачанный PDF файл"):
                cubix_agent_page.cleanup_pdf_after_test()
