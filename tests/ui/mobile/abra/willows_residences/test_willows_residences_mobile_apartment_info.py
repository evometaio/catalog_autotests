...
@pytest.mark.mobile
def test_willows_residences_mobile_apartment_info(mobile_page):
    """Тест отображения информации об аппарте для проекта Willows Residences на мобильном устройстве."""
    ...
    with allure.step("Ищем и кликаем на первый доступный аппарт"):
        ...
        mobile_page.page.wait_for_timeout(2000)  # Ждем загрузки страницы
    ...
    with allure.step("Проверяем отображение информации об аппарте"):
        ...
        apartment_title = mobile_page.page.locator('[class*="propertyInfoTitle"]')
    ...
    with allure.step("Проверяем наличие обертки с информацией"):
        # Используем более общий селектор
        info_wrapper = mobile_page.page.locator('[class*="propertyInfoWrapper"]')
