...
class TestArishaMobileFullNavigationDebug:
    ...
    def test_arisha_mobile_full_navigation_debug(self, mobile_map_page):
        ...
        def debug_locators_on_page(step_name: str):
            ...
            locators_to_check = {
                "building_nav_button": locators.get("BUILDING_NAV_BUTTON"),
                "floor_nav_button": locators.get("FLOOR_NAV_BUTTON"),
                "apartment_nav_button": locators.get("APARTMENT_NAV_BUTTON"),
                "building_1_button": locators.get("BUILDING_1_BUTTON"),
                "building_2_button": locators.get("BUILDING_2_BUTTON"),
                "floor_1_button": locators.get("FLOOR_1_BUTTON"),
                "floor_2_button": locators.get("FLOOR_2_BUTTON"),
                "floor_plan_apartments": locators.get("FLOOR_PLAN_APARTMENTS"),
                "all_buttons": "button",
            ...
            for locator_name, selector in locators_to_check.items():
                try:
                    ...
                    print(
                        f"{status} {locator_name}: {count} элементов, {visible_count} видимых"
                    )

                    if visible_count > 0 and locator_name in [
                        "building_nav_button",
                        "floor_nav_button",
                        "apartment_nav_button",
                    ]:
        ...
        with allure.step("2. Пытаемся найти и кликнуть на кнопку Building Navigation"):
            ...
            try:
                ...
                building_nav = mobile_map_page.page.locator(
                    locators.get("BUILDING_NAV_BUTTON")
                )
        ...
        with allure.step("3. Пытаемся выбрать здание 1"):
            try:
                building_1 = mobile_map_page.page.locator(
                    locators.get("BUILDING_1_BUTTON")
                )
                if building_1.is_visible():
                    ...
                    # Ждем изменения URL
                    try:
                        mobile_map_page.page.wait_for_url(
                            "**/building/1", timeout=10000
                        )
                        current_url = mobile_map_page.get_current_url()
                        print(f"✅ URL изменился на здание 1: {current_url}")
                        allure.attach(
                            f"URL после выбора здания 1: {current_url}",
                            name="Building 1 URL",
                        )
                    except:
        ...
        with allure.step("4. Пытаемся найти и кликнуть на кнопку Floor Navigation"):
            try:
                floor_nav = mobile_map_page.page.locator(
                    locators.get("FLOOR_NAV_BUTTON")
                )
        ...
        with allure.step("5. Пытаемся выбрать этаж 1"):
            try:
                floor_1 = mobile_map_page.page.locator(locators.get("FLOOR_1_BUTTON"))
                if floor_1.is_visible():
                    ...
                    try:
                        ...
                        allure.attach(
                            f"URL после выбора этажа 1: {current_url}",
                            name="Floor 1 URL",
                        )
                    except:
        ...
        with allure.step("6. Ищем апартаменты на плане этажа"):
            try:
                ...
                if apartment_count > 0:
                    with allure.step(
                        "7. Пытаемся кликнуть на первый доступный апартамент"
                    ):
                        try:
                            apartment_clicked = (
                                mobile_map_page.click_available_apartment()
                            )
